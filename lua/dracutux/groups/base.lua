...
function M.get(p)
	...
	return {
		...
		DiffAdd = { bg = util.blend(p.green, p.base, 0.25) },
		DiffChange = { bg = util.blend(p.cyan, p.base, 0.25) },
		DiffDelete = { bg = util.blend(p.red, p.base, 0.25) },
		DiffText = { bg = util.blend(p.comment, p.base, 0.25) },
		...
		MoreMsg = { fg = p.purple },
		NonText = { fg = util.blend(p.subtle_text, p.ui_selection, 0.25) },
		NormalFloat = { fg = p.text, bg = p.ui_base },
		Pmenu = { bg = p.ui_base },
		PmenuSel = { bg = p.ui_selection, bold = true },
		PmenuThumb = { bg = p.subtle_text },
		Question = { fg = p.cyan },
